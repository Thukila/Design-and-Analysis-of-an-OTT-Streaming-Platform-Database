...
on u.user_id=s.user_id 
join Plan as p
on s.plan_id=p.id where plan_name ="Premium";
...
select count(u.user_id) as totaluser, p.plan_name 
from users as u 
left join Subscriptions as s 
on u.user_id=s.user_id 
right join plan as p 
on s.plan_id=p.id 
group by plan_name;
...
select title,duration from content where duration>160;
...
-- 16.Find users who paid more than the average payment amount.
 select user_id,amount from payments where amount > (select avg(amount) as avgamt from payments);
 
 -- 17.Find users who paid more than the average payment amount also print the name
 select u.user_id,u.name,p.amount 
 from users as u 
 join payments as p 
 on u.user_id=p.user_id 
 where amount > (select avg(amount) as avgamt from payments);
...
-- 21.Find users subscribed to the most popular plan.
select title,
	   language,
       duration, 
       RANK() over(
...
select logout_time, ifnull(logout_time,"still active") as session_status from active_sessions;

-- 25. Create a table with CHECK constraint ensuring age is at least 18.
create table age_table ( user_name varchar(50), age int check(age >= 18));
...
select title,length(title) as len_title from content;

-- 28. Display the current system date.
select current_date();

-- 29. Find subscription duration in days.
select id,end_date,start_date, datediff(end_date,start_date) as total_dates from subscriptions;

-- 30. Display 5 most recently released contents after skipping first 3.
select content_id,title,release_date from content order by release_date desc limit 5 offset 3;
